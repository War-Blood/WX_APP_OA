#!/usr/bin/env python3
"""Replace exportStatusBoardCSV function with xlsx version."""
import sys

NEW_FUNC = r'''async function exportStatusBoardCSV(month) {
  ...
  // 4. Query attendance schedules for color coding
  ...
  // 6. Fill daily data with color coding
  const overtime = {};
  const colorMap = {
    '\u73b0\u573a\uff08\u9646\uff09': 'FFC6EFCE', '\u73b0\u573a\uff08\u6d77\uff09': 'FFA9D18E',
    '\u5de5\u4f5c\uff08\u9646\uff09': 'FFC6EFCE', '\u5de5\u4f5c\uff08\u6d77\uff09': 'FFA9D18E',
    '\u5728\u9014': 'FFFFD700', '\u5f85\u5de5': 'FFFFF2CC',
    '\u8bf7\u5047': 'FFFF9999', '\u4f11\u606f': 'FFD9D9D9',
    '\u672a\u63d0\u4ea4': 'FFFFB6C1', '\u5de5\u4f5c': 'FFC6EFCE',
  };
  ...
  days.forEach(dateStr => {
    ...
    activeWorkers.forEach(w => {
      ...
      const schedKey = w.id + '_' + dateStr;
      const sched = schedMap[schedKey];
      ...
      if (!displayStatus && sched) {
        switch (sched) {
          case 'work': displayStatus = '\u5de5\u4f5c'; break;
          case 'biz_trip': displayStatus = '\u5728\u9014'; break;
          case 'rest': displayStatus = '\u4f11\u606f'; break;
          case 'leave': displayStatus = '\u8bf7\u5047'; break;
          default: displayStatus = '\u4f11\u606f';
        }
      }
      if (!displayStatus) displayStatus = '\u4f11\u606f';

      rowData.push(displayDate, displayProject, displayStatus);

      const isOvertime = (
        displayStatus === '\u73b0\u573a\uff08\u9646\uff09' || displayStatus === '\u73b0\u573a\uff08\u6d77\uff09' ||
        displayStatus === '\u5de5\u4f5c' || displayStatus === '\u5de5\u4f5c\uff08\u9646\uff09' || displayStatus === '\u5de5\u4f5c\uff08\u6d77\uff09' ||
        displayStatus === '\u5728\u9014' || displayStatus === '\u5f85\u5de5' ||
        (!info && sched && (sched === 'work' || sched === 'biz_trip'))
      );
      if (isOvertime) overtime[w.id] = (overtime[w.id] || 0) + 1;
    });

    const dataRow = ws1.addRow(rowData);
    activeWorkers.forEach((w, i) => {
      const statusCell = dataRow.getCell(i * 3 + 3);
      const s = statusCell.value || '\u4f11\u606f';
      statusCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: colorMap[s] || 'FFFFFFFF' } };
      for (let k = 1; k <= 3; k++) {
...
old_start = content.index('async function exportStatusBoardCSV')
depth = 0
started = False
old_end = old_start
for idx in range(old_start, len(content)):
    if content[idx] == '{': 
        depth += 1
        started = True
    elif content[idx] == '}':
