...
async function getAllStats() {
  // 全系统日志总条数（排除公司日报）
  const totalRows = await db.query(
    "SELECT COUNT(*) AS cnt FROM daily_reports WHERE report_type != 'office'"
  );
  ...
  // 延迟总条数
  const delayedRows = await db.query(
    "SELECT COUNT(*) AS cnt FROM daily_reports WHERE timeliness = 'delayed'"
  );
  const delayedTotal = delayedRows[0].cnt;

  // 当天有缺失的在职人员数（有 worker_status='active'，但今天没有提交任何日志）
  ...
  const missingRows = await db.query(
    `SELECT COUNT(*) AS cnt FROM users u
     WHERE u.worker_status = 'active'
       ...
       AND u.id NOT IN (
         SELECT dr.user_id FROM daily_reports dr WHERE dr.report_date = ?
         UNION
