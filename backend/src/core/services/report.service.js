...
async function schedulePreview(month) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const status = schedMap[dateStr] || (dayOfWeek === 0 || dayOfWeek === 6 ? 'rest' : 'work');
    days.push({ date: dateStr, dayOfWeek, status });
  }

  return { month, days, workDays: days.filter(d => d.status !== 'rest' && d.status !== 'leave').length, restDays: days.filter(d => d.status === 'rest').length };
}
