...
async function login(code) {
  ...
  // 3. 未注册 → 自动创建 pending 状态用户
  if (users.length === 0) {
    logger.info('微信登录 - 自动注册新用户', { module: 'AUTH', openid });

    const result = await db.execute(
      `INSERT INTO users (openid, user_name, role, status, created_at) VALUES (?, '新用户', 'employee', 'pending', NOW())`,
      [openid]
    );
    ...
    await db.execute(
      `INSERT INTO operation_logs (user_id, action, module, detail, created_at) VALUES (?, 'auto_register', 'AUTH', '新用户自动注册，等待审核', NOW())`,
      [newUser.id]
    );

    logger.info('新用户已注册，等待审核', { module: 'AUTH', userId: newUser.id });
    throw new BusinessError('您的账号已提交，请等待管理员审核通过后重新登录');
  }
  ...
  if (user.status === 'pending') {
    throw new BusinessError('您的账号正在审核中，请等待管理员审核通过');
  }
