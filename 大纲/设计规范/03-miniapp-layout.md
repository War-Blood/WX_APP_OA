...
| 项目 | 行业标准 |
|------|---------|
| 设计画布 | 375×667px（1x），导出 750×1334px（2x） |
| 屏幕基准 | 750rpx = 屏幕宽度 |
| 导航栏高度 | 128rpx（44-48px），标题居中 |
| 底部标签栏 | 98rpx（最多 5 标签） |
| iPhone 安全区 | 34px（`env(safe-area-inset-bottom)`） |
| 最小点击区 | 88rpx × 88rpx |
...
.btn-outline:active { background: #F0F2F5; }
```

> 参考来源：微信小程序官方设计指南 + 行业最佳实践
