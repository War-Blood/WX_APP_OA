...
|------|------|------|
| 名称 | el-input | 展示名 |
| env 引用名 | el-input | 如 `DAILY`，tooltip 提示对应 `.env` 的 `WECOM_ROBOT_DAILY_KEY/_SECRET`；**无凭证输入框**，标注"凭证由服务端 .env 管理（对齐 WPS）" |
| 凭证状态 | el-tag | 已配置（green）/ 未配置（red，tooltip：请运维在 .env 补充并重启） |
| 启用 | el-switch | 未配置凭证时禁用开关并提示 |
| 备注 | el-input | ≤255 |

- 删除保护：被脚本引用时后端拒绝，前端提示"请先修改引用该机器人的脚本"。
- 文案区：安全说明卡（加签、IP 白名单运维指引）。
