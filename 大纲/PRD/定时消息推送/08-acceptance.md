...
- [ ] 凭证零回显：创建后列表/详情接口只返回脱敏 key（保留后 4 位）；遍历 API 响应、前端页面、日志文件，无完整 webhook key/secret；错误信息 URL 脱敏。
- [ ] 设置方式：仅「名称 + Webhook」两字段必填即可创建并直接发送成功；非法 URL/key 被表单与后端双重拦截。
- [ ] 编辑留空：编辑时不填 Webhook 不覆盖原 key；填写新值则更新。
- [ ] 加签正确：配置 secret 后 sign 计算符合企微规范（HMAC-SHA256 + base64 + `timestamp\nsecret`）；错误 secret 的请求被企微 93000 拒绝（联调验证）；未配置 secret 时普通发送成功（兼容未加签机器人）。
- [ ] 固定 URL：出站 URL 固定企微域名；key/secret 格式校验 `^[A-Za-z0-9\-_]{8,}$` 生效。
