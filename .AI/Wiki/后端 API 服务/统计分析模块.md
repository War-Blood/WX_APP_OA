...
- [wps.controller.js](file://backend/src/features/controllers/wps.controller.js)
- [wps.service.js](file://backend/src/features/services/wps.service.js)
- [wps.routes.js](file://backend/src/features/routes/wps.routes.js)
...
- **新增** WPS 集成：为 WPS 表格提供直接的 JSON 和 CSV 数据接口

## 项目结构
统计分析模块主要分布在后端 features 与 core 两大领域层，并通过统一的响应工具与数据库配置进行协作。新增的 WPS 集成功能提供了独立的控制器、服务和路由模块。
...
subgraph "WPS 集成接口层"
RWPS["wps.routes.js<br/>/api/wps/*"]
CWPS["wps.controller.js<br/>控制器"]
end
subgraph "统计服务层"
SStats["stats.service.js<br/>统计服务"]
end
subgraph "WPS 服务层"
SWPS["wps.service.js<br/>WPS 服务"]
end
subgraph "核心业务接口层"
...
- [wps.routes.js:1-31](file://backend/src/features/routes/wps.routes.js#L1-L31)
- [wps.controller.js:1-36](file://backend/src/features/controllers/wps.controller.js#L1-L36)
...
- **新增** WPS 控制器：提供 WPS 表格数据接口，支持直接 JSON 和 CSV 格式输出
- **新增** WPS 服务：实现 WPS 数据格式转换，提供审核通过的日报数据
...
### WPS 数据接口概述
新增的 WPS 集成功能为外部办公软件 WPS 提供了专门的数据接口，支持直接 JSON 和 CSV 格式的日报数据输出，便于 WPS 表格和多维表格的集成使用。

### WPS JSON 接口
- 接口：GET /api/wps/reports
- 请求参数：
  - key：API 密钥（查询参数或请求头 x-api-key）
- 返回字段：直接的 JSON 数组，包含格式化的日报数据
- 特殊处理：WPS 表格需要直接的数组格式，不要外层包装

实现要点：
- 仅返回状态为"approved"的审核通过日报
- 支持中文字段名：日报时间、填写人、入场时间等
- 限制返回 5000 条记录
- 使用自定义日期格式化函数

### WPS CSV 接口
- 接口：GET /api/wps/reports.csv
- 请求参数：
  - key：API 密钥（查询参数或请求头 x-api-key）
- 返回：CSV 文件（UTF-8 BOM），供 WPS 多维表格导入
- 特殊处理：添加 UTF-8 BOM 头，便于 Excel/WPS 正确识别编码

实现要点：
- 使用与 JSON 接口相同的数据源和格式化规则
- 实现 CSV 转义处理：含逗号、换行、引号时用双引号包裹
- 自动添加文件名头部信息

### API Key 鉴权机制
- 鉴权方式：基于环境变量的静态密钥验证
- 配置要求：需要设置 WPS_API_KEY 环境变量
- 错误处理：未配置时返回 500 错误，密钥错误时返回 401 错误

```mermaid
sequenceDiagram
participant WPS as "WPS 客户端"
participant Router as "wps.routes.js"
participant Auth as "apiKeyAuth"
participant Controller as "wps.controller.js"
participant Service as "wps.service.js"
WPS->>Router : GET /api/wps/reports?key=xxx
Router->>Auth : 验证 API 密钥
Auth->>Auth : 检查 WPS_API_KEY 环境变量
Auth->>Controller : 验证通过
Controller->>Service : getReports()
Service->>Service : 格式化数据
Service-->>Controller : 返回 JSON 数组
Controller-->>WPS : 直接 JSON 响应
```

**图表来源**
- [wps.routes.js:10-21](file://backend/src/features/routes/wps.routes.js#L10-L21)
- [wps.controller.js:10-18](file://backend/src/features/controllers/wps.controller.js#L10-L18)
...
WPSCtrl["wps.controller.js"] --> WPSSvc["wps.service.js"]
...
- **新增** WPS 接口优化：限制返回记录数为 5000 条，避免大数据量传输影响性能
...
- **新增** WPS 接口问题
  - 现象：WPS 无法获取数据或出现乱码
  - 排查：检查 WPS_API_KEY 环境变量配置、CSV 编码问题、API 密钥传递方式
...
- [wps.routes.js:14-21](file://backend/src/features/routes/wps.routes.js#L14-L21)

## 结论
统计分析模块以清晰的三层架构实现首页统计、个人统计、动态聚合与日报看板等核心能力，配合并行查询与分页机制保障性能与体验。**新增的 WPS 集成功能**为外部办公软件提供了标准化的数据接口，支持直接 JSON 和 CSV 格式输出，便于与其他办公工具集成。未来可在现有基础上扩展"审批统计"与"高级分析"能力，并引入 Redis 缓存以进一步提升热点数据的响应速度。
...
- **新增** WPS JSON 接口
  - 方法：GET
  - 路径：/api/wps/reports
  - 参数：key（查询参数或请求头 x-api-key）
  - 响应：直接的 JSON 数组（5000 条记录限制）
- **新增** WPS CSV 接口
  - 方法：GET
  - 路径：/api/wps/reports.csv
  - 参数：key（查询参数或请求头 x-api-key）
  - 响应：HTTP 200 + CSV 文件（UTF-8 BOM）
...
- [wps.routes.js:27-28](file://backend/src/features/routes/wps.routes.js#L27-L28)
- [wps.controller.js:10-33](file://backend/src/features/controllers/wps.controller.js#L10-L33)
...
- **新增** WPS 数据格式化：提供中文字段名、日期格式化、数值处理等
...
- [wps.service.js:8-56](file://backend/src/features/services/wps.service.js#L8-L56)
...
- **新增** 外部系统集成：为第三方办公软件提供标准化 API 接口，支持多种数据格式输出