...
```json
{ "code": 0, "message": "success", "data": { "userId": "U10010" } }
```
...
```json
{
  ...
  "data": {
    "list": [
      { "userId": "U10001", "name": "张三", "department": "技术部" }
    ]
  }
}
```
...
```json
{
  ...
  "data": {
    "list": [
      { "userId": "U10001", "name": "张三", "submitCount": 20, "totalCount": 22 }
    ]
  }
}
```
...
## 9. 公告模块 (Announcement) ⏳ 待实现

> **状态**: 后端路由未实现。以下接口定义为规划文档，开发前请先实现 `backend/src/core/routes/announcement.routes.js`。
...
> **状态**: 后端路由未实现。以下接口定义为规划文档，开发前请先实现 `backend/src/core/routes/asset.routes.js`。
...
```json
{
  "code": 0, "message": "success",
  "data": {
    "summary": { "totalUsers": 50, "submittedUsers": 45, "totalReports": 200 },
    "list": [{ "userId": "U10001", "name": "张三", "submitRate": 0.95 }]
  }
}
```
