...
## 更新摘要
**变更内容**
- 新增WPS集成功能，支持与WPS表格的数据对接
- 扩展消息类型支持，增加report、task等新类型
- 增强消息服务功能，支持更丰富的消息处理逻辑
- 更新消息路由和控制器，提供完整的消息管理API
...
## 简介
本文件为"智慧办公助手 OA 系统"的消息通知模块提供全面技术文档。内容覆盖消息系统架构设计、消息类型分类与处理机制、消息模板与发送策略、消息状态追踪与历史管理，并对消息相关 API 进行逐项说明。同时新增了WPS集成功能，支持与WPS表格的数据对接，为企业办公自动化提供完整解决方案。文档涵盖消息列表、详情、未读统计、标记已读、删除等端点的调用方式与数据模型，辅以关键流程图与时序图帮助理解。

## 项目结构
消息模块在后端采用经典的三层结构：路由层负责接口定义与鉴权；控制器层承接请求并组织响应；服务层封装数据访问与业务逻辑；前端通过服务模块调用后端接口，页面组件负责渲染与交互。新增的WPS集成功能独立于消息模块，提供专门的API接口用于数据交换。
...
WPSRoutes["WPS路由<br/>wps.routes.js"]
WPSController["WPS控制器<br/>wps.controller.js"]
WPSService["WPS服务<br/>wps.service.js"]
...
Routes --> Ctrl
Ctrl --> Svc
WPSRoutes --> WPSController
WPSController --> WPSService
Svc --> DB
WPSService --> DB
DB --> Models
```
...
- [wps.routes.js:1-30](file://backend/src/features/routes/wps.routes.js#L1-L30)
...
- **消息服务层**：封装数据库访问、消息格式化、分页计算、未读计数、标记已读、删除等逻辑，支持多种消息类型。
- **WPS集成层**：提供API Key鉴权的WPS数据接口，支持JSON和CSV格式的数据导出。
...
## 架构总览
消息模块遵循"前后端分离 + 统一响应 + 统一鉴权"的设计原则。后端通过 JWT 中间件进行身份校验与用户信息挂载，所有消息接口均需携带有效 Bearer Token。服务层对数据库进行参数化查询，避免注入风险；前端通过服务模块发起请求，页面组件负责展示与交互。新增的WPS集成功能通过独立的路由和控制器处理外部系统的数据请求。
...
### 消息类型与模板系统
- **消息类型**：后端定义了approval、system、announcement、report、task等多种类型枚举，前端页面根据类型映射图标与背景色，支持按类型筛选。
- **模板字段**：消息列表与详情在服务层进行字段映射与格式化，包括标题、摘要、时间、是否已读、图标与背景色、详情正文、动作文本与路由等。
- **图标映射**：不同消息类型对应不同的图标和背景色，提供直观的视觉识别。
...
### 消息发送策略与去重
- **发送策略**：消息表包含发送者、接收者、类型、标题、内容、关联业务标识等字段，便于后续扩展为审批通知、公告推送、任务提醒等场景。
...
### API Key 鉴权机制
WPS集成功能通过独立的路由和控制器实现，采用API Key方式进行鉴权，确保只有授权的外部系统可以访问数据。

```mermaid
sequenceDiagram
participant WPS as "WPS系统"
participant API as "WPS路由<br/>wps.routes.js"
participant AUTH as "API Key鉴权<br/>apiKeyAuth"
participant CTRL as "WPS控制器<br/>wps.controller.js"
participant SRV as "WPS服务<br/>wps.service.js"
participant DB as "数据库<br/>database.js"
WPS->>API : GET /api/wps/reports?key=xxx
API->>AUTH : 验证API Key
AUTH->>AUTH : 检查process.env.WPS_API_KEY
AUTH->>CTRL : 鉴权通过
CTRL->>SRV : getReports()
SRV->>DB : 查询已审核通过的日报数据
DB-->>SRV : 返回数据
SRV-->>CTRL : 格式化后的JSON数组
CTRL-->>WPS : 直接返回数组格式数据
```
...
- **CSV数据接口**
  ...
  - 参数：key=API密钥
  - 响应：CSV格式的已审核通过日报数据，支持Excel/WPS导入
  - 特点：包含BOM头，兼容中文编码

### 数据格式与转换
WPS服务将数据库中的日报数据转换为WPS表格友好的格式，包括中文字段名和适当的数值格式处理。
...
WPSAuth["API Key鉴权<br/>wps.routes.js"] --> WPSRoutes["WPS路由<br/>wps.routes.js"]
WPSRoutes --> WPSController["WPS控制器<br/>wps.controller.js"]
WPSController --> WPSService["WPS服务<br/>wps.service.js"]
...
- **WPS API Key配置问题**
  - 现象：返回"WPS API 未配置"或"无效的 API 密钥"。
  - 排查：确认process.env.WPS_API_KEY已正确设置，API Key与请求参数匹配。
  - 参考
...
## 结论
消息通知模块以清晰的分层架构实现了消息的接收、展示、状态管理与历史维护。后端通过统一响应与鉴权中间件保障安全性与一致性，前端页面提供了良好的交互体验。新增的WPS集成功能进一步扩展了系统的数据交换能力，支持与WPS表格的无缝对接。建议在生产环境中完善消息去重策略与索引设计，持续优化分页与查询性能，同时确保WPS API Key的安全配置。
...
### WPS数据接口规范
- **JSON接口**：直接返回数组格式的JSON数据，适合WPS表格直接引用
- **CSV接口**：返回CSV格式数据，支持Excel/WPS导入，包含UTF-8 BOM头
- **数据限制**：最多返回5000条已审核通过的日报记录
- **字段映射**：将英文字段名转换为中文字段名，便于WPS表格理解
