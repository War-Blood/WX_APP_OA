...
## 简介
本文件面向“智慧办公助手 OA 系统”的后端 API 服务，采用 Node.js + Express 构建 RESTful API。系统围绕认证授权、审批管理、日报管理、消息通知、统计分析、审核管理等核心业务模块进行组织，提供统一的响应格式、完善的中间件体系与错误处理机制。本文档旨在帮助开发者快速理解系统架构、API 设计原则与调用方式，并提供可追溯的代码路径以供参考。

## 项目结构
后端采用“按功能域分层 + 路由聚合”的组织方式：
...
- 功能域模块：
  ...
  - 特性域：features 路由聚合，包含统计分析、WPS 数据接口等。
...
- 统一响应与常量
  ...
  - 常量：ErrorCode、Role、UserStatus、ApprovalType/Status、MessageType、分页默认值、业务常量。
...
## 架构总览
系统采用“入口 -> 中间件 -> 路由 -> 控制器 -> 服务 -> 数据库/外部服务”的标准 Express 架构。认证中间件贯穿核心业务，确保所有受保护接口均需有效 Token；错误处理中间件保证统一响应格式与日志记录；Swagger 文档辅助前后端协作。
...
RT_I --> RT_R["report.routes.js"]
RT_I --> RT_A["approval.routes.js"]
...
## 结论
本系统以清晰的模块划分与中间件体系为基础，结合统一响应与错误处理机制，实现了认证授权、审批管理、日报管理、消息通知、统计分析等核心业务的 RESTful API。通过 Swagger 文档与严格的参数校验、权限控制与限流策略，提升了系统的安全性与可维护性。建议在后续迭代中持续完善单元测试覆盖率与接口契约文档，保障跨端协同效率。
