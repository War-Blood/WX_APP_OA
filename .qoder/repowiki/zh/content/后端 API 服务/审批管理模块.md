...
## 简介
本文件面向“智慧办公助手 OA 系统”的审批管理模块，系统性阐述审批流程设计与实现，覆盖请假、出差、采购、费用报销等审批类型的配置与运行机制；详解审批状态流转、审批人分配策略、抄送功能、审批历史追踪；并提供完整的 API 接口清单与调用示例路径，帮助开发者快速集成与扩展复杂审批场景（如并行审批、串行审批、条件审批等）。
...
DB["数据库表<br/>approval_instances / approval_flow_nodes / approval_cc"]
end
A_index --> A_api
A_create --> A_api
A_detail --> A_api
...
- 数据模型：审批实例、审批流程节点、抄送关系三张核心表，支撑状态机与历史追踪。
